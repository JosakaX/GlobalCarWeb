    ...
    fetch(`/api/vehicles${queryString}`)
      ...
      .then((vehicles) => {
        ...
        if (vehicles.length === 0) {
          carGrid.innerHTML =
            '<div class="col-span-1 md:col-span-2 lg:col-span-3 text-center py-12"><p class="text-xl text-gray-500 font-medium">No se encontraron vehículos.</p></div>';
  ...
  if (urlParams.has("make") && searchSelect) {
    ...
    if (searchSelect.options) {
      for (let i = 0; i < searchSelect.options.length; i++) {
        if (
          searchSelect.options[i].value.toLowerCase() ===
...
window.fetchBrandsAndPopulate = function () {
  fetch("/api/marcas")
    ...
    .then((marcas) => {
      ...
      heroSelects.forEach((select) => {
        ...
        if (urlParams.has("make")) {
          ...
          for (let i = 0; i < select.options.length; i++) {
            if (
              select.options[i].value.toLowerCase() === makeValue.toLowerCase()
            ) {
      ...
      // Update Pills (Catalog - interactive)
      ...
      // Update Pills (Index - decorative only)
      const group1Index = document.getElementById("brand-scroll-group-1-index");
      const group2Index = document.getElementById("brand-scroll-group-2-index");
      if (group1Index && group2Index) {
        let htmlIndex = "";
        marcas.forEach((marca) => {
          htmlIndex += `
                        <div class="flex-shrink-0 bg-[#002f6c] rounded-full border-[3px] border-[#0047b3] shadow-[0_0_0_3px_#ff6600,0_4px_10px_rgba(0,0,0,0.2)] flex items-center justify-center min-w-[190px] h-[65px] px-6 relative overflow-hidden transition-colors">
                            <div class="absolute inset-0 bg-gradient-to-b from-white/10 to-transparent pointer-events-none rounded-full"></div>
                            <span class="text-white font-black text-xl tracking-widest uppercase">${marca.nombre}</span>
                        </div>
                    `;
        });
        group1Index.innerHTML = htmlIndex;
        group2Index.innerHTML = htmlIndex;
      }
    })
